...
def gsf_read(file_name):
    ...
    if 'XReal' in metadata:
        metadata['XReal'] = np.float32(metadata['XReal'])
    
    if 'YReal' in metadata:
        metadata['YReal'] = np.float32(metadata['YReal'])
                
    if 'XOffset' in metadata:
        metadata['XOffset'] = np.float32(metadata['XOffset'])
    
    if 'YOffset' in metadata:
        metadata['YOffset'] = np.float32(metadata['YOffset'])
